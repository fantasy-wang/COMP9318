...
def make_N_days_data(df):
    elements = ['max_temp', 'max_dew', 'max_humid', 'dailly_cases']
...
def get_interval_data(df, weather_interval, cases_interval):
    subset_weather = ['max_temp', 'max_dew', 'max_humid']
...
def predict_COVID_part1(svm_model, train_df, train_labels_df, past_cases_interval, past_weather_interval, test_feature):
    train_df_interval = get_interval_data(make_N_days_data(train_df), past_weather_interval, past_cases_interval)
